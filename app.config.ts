...
export default ({ config }: ConfigContext): ExpoConfig => {
  const expoProjectId = process.env.EXPO_PROJECT_ID ?? 'bb51714e-310f-4266-8c47-6d1f33a2bbc9';
  const expoConfig: ExpoConfig = {
    ...
    ios: {
      ...config.ios,
      bundleIdentifier:
        process.env.EXPO_IOS_BUNDLE_IDENTIFIER ?? 'com.abizera.react-native-boilerplate',
    },
    android: {
      ...config.android,
      package: process.env.EXPO_ANDROID_PACKAGE ?? 'com.abizera.react_native_boilerplate',
    },
    web: {
      ...config.web,
    },
